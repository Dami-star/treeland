...
private:
    ...
    void waitForCleanup(int cycles = CLEANUP_CYCLES, int sleepMs = CLEANUP_SLEEP_MS)
    {
    ...
    TreelandUserConfig *createConfig()
    {
    ...
    void initTestCase()
    {
        ...
        if (!s_initialized) {
            ...
            if (testConfig) {
                ...
                int waitCount = 0;
                while (!testConfig->isInitializeSucceed() && waitCount < INIT_TIMEOUT_ITERATIONS) {
