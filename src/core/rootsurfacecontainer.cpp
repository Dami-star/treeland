...
void RootSurfaceContainer::setupSeatManagement()
{
    ...
    // TODO: WSeatManager currently does not provide seatAdded/seatRemoved signals.
    // the signal infrastructure would need to be added to WSeatManager.
}
...
void RootSurfaceContainer::onSeatAdded(WSeat *seat)
{
    ...
    auto *container = new SeatSurfaceContainer(seat, this);
    m_seatContainers[seat] = container;

    connect(container, &SeatSurfaceContainer::moveResizeChanged, this, &RootSurfaceContainer::moveResizeFinised);
}
...
SeatSurfaceContainer *RootSurfaceContainer::getSeatContainer(WSeat *seat) const
{
...
SeatSurfaceContainer *RootSurfaceContainer::getSeatContainerOrDefault(WSeat *seat) const
{
    if (!seat) {
        seat = getDefaultSeat();
        if (!seat) {
