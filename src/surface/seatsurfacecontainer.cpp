...
#include "seatsurfacecontainer.h"
...
SeatSurfaceContainer::SeatSurfaceContainer(WSeat *seat, RootSurfaceContainer *parent)
    : QObject(parent)
    , m_seat(seat)
...
SeatSurfaceContainer::~SeatSurfaceContainer()
{
    endMoveResize();
}

void SeatSurfaceContainer::setActivatedSurface(SurfaceWrapper *surface, Qt::FocusReason reason)
{
    Q_UNUSED(reason);
...
void SeatSurfaceContainer::setKeyboardFocusSurface(SurfaceWrapper *surface)
{
    ...
    if (oldSurface) {
        ...
        if (auto *helper = Helper::instance()) {
            if (auto *seatManager = helper->seatManager()) {
                for (auto *otherSeat : seatManager->seats()) {
                    if (otherSeat == m_seat)
                        continue;
                    if (auto *otherContainer = helper->rootSurfaceContainer()->getSeatContainer(otherSeat)) {
                        if (otherContainer->keyboardFocusSurface() == oldSurface) {
...
void SeatSurfaceContainer::beginMoveResize(SurfaceWrapper *surface, Qt::Edges edges)
{
...
void SeatSurfaceContainer::doMoveResize(const QPointF &delta)
{
    if (!m_moveResizeState.surface)
        return;
...
void SeatSurfaceContainer::endMoveResize()
{
    if (!m_moveResizeState.surface)
        return;
...
SurfaceWrapper *SeatSurfaceContainer::moveResizeSurface() const
{
    return m_moveResizeState.surface;
}

void SeatSurfaceContainer::cancelMoveResize(SurfaceWrapper *surface)
{
...
bool SeatSurfaceContainer::shouldHandleShortcuts() const
{
...
void SeatSurfaceContainer::setMetaKeyPressed(bool pressed)
{
...
void SeatSurfaceContainer::surfaceDestroyed(SurfaceWrapper *surface)
{
    if (m_moveResizeState.surface == surface) {
        endMoveResize();
    }
