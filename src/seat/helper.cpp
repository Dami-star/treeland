...
void Helper::init(Treeland::Treeland *treeland)
{
    ...
    if (!m_seat) {
        qCCritical(treelandCore) << "No seat available after initSeatRuntime, cannot continue";
        return;
    }
...
void Helper::initSeatConfig()
{
    ...
    if (m_seatManager->seats().isEmpty()) {
        ...
        m_seat = defaultSeat;
        m_seatManager->addDeviceRule("seat0", QString("%1:.*").arg(static_cast<int>(WInputDevice::Type::Keyboard))); // Keyboard devices
        m_seatManager->addDeviceRule("seat0", QString("%1:.*").arg(static_cast<int>(WInputDevice::Type::Pointer))); // Pointer devices
        m_seatManager->addDeviceRule("seat0", QString("%1:.*").arg(static_cast<int>(WInputDevice::Type::Touch))); // Touch devices
    } else {
        m_seat = m_seatManager->fallbackSeat();
        if (!m_seat) {
            m_seat = m_seatManager->seats().first();
        }
        if (m_seatManager->deviceRules(m_seat->name()).isEmpty()) {
            m_seatManager->addDeviceRule(m_seat->name(), QString("%1:.*").arg(static_cast<int>(WInputDevice::Type::Keyboard)));
            m_seatManager->addDeviceRule(m_seat->name(), QString("%1:.*").arg(static_cast<int>(WInputDevice::Type::Pointer)));
            m_seatManager->addDeviceRule(m_seat->name(), QString("%1:.*").arg(static_cast<int>(WInputDevice::Type::Touch)));
        }
    }

    for (auto *seat : m_seatManager->seats()) {
        if (seat->server() != m_server) {
            m_server->attach(seat);
        }
    }
}

void Helper::initSeatRuntime()
{
    if (!m_seat) {
        qCCritical(treelandCore) << "initSeatRuntime called with no m_seat!";
        return;
    }
...
WSeat *Helper::getSeatForDevice(WInputDevice *device) const
{
    ...
    if (m_deviceSeatCache.contains(device))
        return m_deviceSeatCache[device];

    WSeat *seat = m_seatManager->findSeatForDevice(device);
    m_deviceSeatCache[device] = seat;

    return seat;
}

void Helper::clearDeviceSeatCache(WInputDevice *device)
{
    m_deviceSeatCache.remove(device);
}
...
void Helper::assignDeviceToSeat(WInputDevice *device)
{
    ...
    if (!assigned && m_seat && m_seat->nativeHandle()) {
        ...
        m_deviceSeatCache[device] = m_seat;
    }
    ...
    m_deviceSeatCache[device] = assignedSeat;
}
...
void Helper::setupSeatsConfiguration()
{
    for (auto *seat : m_seatManager->seats()) {
        if (!seat->nativeHandle()) {
            qCWarning(treelandCore) << "Seat" << seat->name() << "has no native handle, skipping configuration";
            continue;
        }
