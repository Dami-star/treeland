...
Q_LOGGING_CATEGORY(waylibSeatManager, "waylib.server.seatmanager", QtInfoMsg)
    ...
    if (m_seats.contains(name)) {
        ...
        for (auto device : devices) {
            ...
            if (!m_seats.isEmpty()) {
                ...
                if (!assigned) {
                    qCWarning(waylibSeatManager) << "Failed to reassign device after seat removal:" << device->name();
                    // Assign to fallback seat if available
                    if (auto fallback = fallbackSeat()) {
                        fallback->attachInputDevice(device);
                        qCDebug(waylibSeatManager) << "Device assigned to fallback seat:" << device->name();
                    }
                }
            }
        }
...
void WSeatManager::removeSeat(WSeat *seat)
{
    ...
    if (!seatName.isEmpty()) {
        ...
        qCWarning(waylibSeatManager) << "Attempted to remove a seat that is not managed by WSeatManager";
    }
}
...
{
    if (!device) {
        qCWarning(waylibSeatManager) << "Cannot assign null device to seat";
        return;
    }
...
{
    if (!device) {
        qCWarning(waylibSeatManager) << "Cannot auto-assign null device";
        return false;
    }
...
{
    if (!output) {
        qCWarning(waylibSeatManager) << "Cannot assign null output to seat";
        return;
    }
...
{
    if (seatName.isEmpty()) {
        qCWarning(waylibSeatManager) << "Cannot add device rule for seat with empty name";
        return;
    }
    
    if (rule.isEmpty()) {
        qCWarning(waylibSeatManager) << "Cannot add empty device rule";
        return;
    }
    
    if (!m_seats.contains(seatName)) {
        qCWarning(waylibSeatManager) << "Cannot add device rule for non-existent seat:" << seatName;
        return;
    }
    
    QRegularExpression regex(rule);
    if (!regex.isValid()) {
        qCWarning(waylibSeatManager) << "Invalid regex pattern for device rule:" << rule << "Error:" << regex.errorString();
        return;
    }
    ...
    for (const auto &existingRule : m_deviceRules[seatName]) {
...
{
    if (!server) {
        qCCritical(waylibSeatManager) << "Cannot create seats with null server";
        return;
    }
    ...
    for (auto it = m_seats.begin(); it != m_seats.end(); ++it) {
        ...
        if (!seat->nativeHandle()) {
            qCCritical(waylibSeatManager) << "Failed to create native handle for seat" << seat->name();
        }
    }
